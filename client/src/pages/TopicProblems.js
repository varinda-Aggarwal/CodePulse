    ...
    const fetchData = async () => {
        ...
        try {
            const [topicsRes, problemsRes] = await Promise.all([
                API.get('/topics'),
                API.get('/problems', { params: { topic: id, limit: 100 } })
            ]);
    ...
    return (
        <div className="min-h-screen bg-gray-900">
            ...
            <div className="max-w-5xl mx-auto p-6">
                ...
                <button onClick={() => navigate('/topics')} className="text-gray-400 hover:text-white mb-4">← Back to Topics</button>

                {selectedProblem ? (
                    <div className="bg-gray-800 rounded-lg p-6">
                        <button onClick={() => { setSelectedProblem(null); setIsEditing(false); }} className="text-gray-400 hover:text-white mb-4">← Back to Problems</button>
                    ...
                    <>
                        <h1 className="text-3xl font-bold text-white mb-6">📁 {topic?.name || 'Topic'} — Problems</h1>

                        <div className="space-y-3">
                            {problems.length === 0 && <p className="text-gray-400 text-center">No problems added under this topic yet.</p>}
